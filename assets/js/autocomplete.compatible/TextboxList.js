...
$j.TextboxList = function(element, _options){
    ...
    plugins: {},
		...
		encode: function(o){ 
			return $j.grep($j.map(o, function(v){		
				v = (chk(v[0]) ? v[0] : v[1]);
	...
	var init = function(){		
		...
		container = $j('<div class="'+options.prefix+'" />')
			.insertAfter(element)
			.click(function(e){ 
		...
		list = $j('<ul class="'+ options.prefix +'-bits" />').appendTo(container);
		for (var name in options.plugins) enablePlugin(name, options.plugins[name]);		
		afterInit();
	};
	
	var enablePlugin = function(name, options){
		self.plugins[name] = new $j.TextboxList[camelCase(capitalize(name))](self, options);
	};
	...
	var afterInit = function(){
		...
		$j(document).click(function(e){
			...
			if (e.target.className.indexOf(options.prefix) != -1){				
				if (e.target == $j(container).get(0)) return;				
				var parent = $j(e.target).parents('div.' + options.prefix);
			...
			var special = !!$j.map(['shift', 'alt', 'meta', 'ctrl'], function(e){ return ev[e]; }).length;
	...
	var create = function(klass, value, opt){
		if (klass == 'box'){
			...
			if (options.unique && $j.inArray(uniqueValue(value), index) != -1) return false;		
		}		
		return new $j.TextboxListBit(klass, value, self, $j.extend(true, options.bitsOptions[klass], opt));
	};
	...
	var focusRelative = function(dir, to){
		var el = getBit(to && $j(to).length ? to : current).toElement();
		var b = getBit(el[dir]());
		if (b) b.focus();
		return self;
	};
	...
	var getBit = function(obj){				
		return (obj.type && (obj.type == 'editable' || obj.type == 'box')) ? obj : $j(obj).data('textboxlist:bit');
	};
	...
	var setValues = function(values){
		...
		$j.each(values, function(i, v){
			if (v) add.apply(self, $j.isArray(v) ? [v[1], v[0], v[2]] : [v]);
		});		
	};
	...
	var fireEvent = function(type, args, delay){
		...
		$j.each(events[type], function(i, fn){		
			(function(){
	...
	var isDuplicate = function(v){
		return $j.inArray(uniqueValue(v), index);
	};
	...
	this.setOptions = function(opt){
		options = $j.extend(true, options, opt);
	};
...
$j.TextboxListBit = function(type, value, textboxlist, _options){
	
	var element, bit, prefix, typeprefix, close, hidden, focused = false, name = capitalize(type); 
	var options = $j.extend(true, type == 'box' ? {
		deleteButton: true
  } : {
		tabIndex: null,
		growing: true,
	...
	var init = function(){
		...
		bit = $j('<li />').addClass(prefix).addClass(typeprefix)
		...
		if (type == 'box'){
			...
			if (options.deleteButton){
				...
				close = $j('<a href="#" class="'+ typeprefix +'-deletebutton" />').click(remove).appendTo(bit);
			}
			bit.children().click(function(e){ e.stopPropagation(); e.preventDefault(); });
		} else {
			element = $j('<input type="text" class="'+ typeprefix +'-input" autocomplete="off" />').val(self.value ? self.value[1] : '').appendTo(bit);
			if (chk(options.tabIndex)) element.tabIndex = options.tabIndex;
			if (options.growing) new $j.GrowingInput(element, options.growingOptions);		
			...
			if (options.addKeys || options.stopEnter){
				element.keydown(function(ev){
					...
					if ($j.inArray(ev.which, splat(options.addKeys)) != -1){
						evStop();
						toBox();
					}
				});
...
var splat = function(a){ return $j.isArray(a) ? a : [a]; };
...
$j.fn.extend({
	...
	textboxlist: function(options){
		return this.each(function(){
			new $j.TextboxList(this, options);
		});
	}
