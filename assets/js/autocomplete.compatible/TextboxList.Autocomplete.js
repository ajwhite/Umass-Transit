...
$j.TextboxList.Autocomplete = function(textboxlist, _options){
  ...
  var index, prefix, method, container, list, values = [], searchValues = [], results = [], placeholder = false, current, currentInput, hidetimer, doAdd, currentSearch, currentRequest;
	...
	var init = function(){
		...
		if ($j.browser.msie) textboxlist.setOptions({bitsOptions: {editable: {addOnBlur: false}}});
		prefix = textboxlist.getOptions().prefix + '-autocomplete';
		method = $j.TextboxList.Autocomplete.Methods[options.method];
		container = $j('<div class="'+ prefix +'" />').width(textboxlist.getContainer().width()).appendTo(textboxlist.getContainer());
		if (chk(options.placeholder)) placeholder = $j('<div class="'+ prefix +'-placeholder" />').html(options.placeholder).appendTo(container);		
		list = $j('<ul class="'+ prefix +'-results" />').appendTo(container).click(function(ev){
	...
	var search = function(bit){
		...
		var search = $j.trim(currentInput.getValue()[1]);
	...
	var showResults = function(search){
		...
		if (textboxlist.getOptions().unique){
			results = $j.grep(results, function(v){ return textboxlist.isDuplicate(v) == -1; });		
		}
		hidePlaceholder();
		if (!results.length) return;
		blur();
		list.empty().css('display', 'block');
		$j.each(results, function(i, r){ addResult(r, search); });
		if (options.onlyFromValues) focusFirst();
		results = results;
	};
	
	var addResult = function(r, searched){
		var element = $j('<li class="'+ prefix +'-result" />').html(r[3] ? r[3] : r[1]).data('textboxlist:auto:value', r);		
		element.appendTo(list);
		if (options.highlight) $j(options.highlightSelector ? element.find(options.highlightSelector) : element).each(function(){
			if ($j(this).html()) method.highlight($j(this), searched, options.insensitive, prefix + '-highlight');
		});
	...
	var addCurrent = function(){
		...
		if (b){
			b.autoValue = value;
			if ($j.isArray(index)) index.push(value);
...
$j.TextboxList.Autocomplete.Methods = {
